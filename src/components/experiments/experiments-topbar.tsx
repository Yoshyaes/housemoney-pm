...
interface ExperimentsTopbarProps {
  isGuest?: boolean;
}

export function ExperimentsTopbar({ isGuest }: ExperimentsTopbarProps) {
  const { setCreateModalOpen } = useExperimentsStore();

  return (
    <div className="flex items-center justify-between border-b border-zinc-200/60 dark:border-zinc-800 px-5 py-3">
      <div>
        <h1 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Experiments</h1>
        <p className="text-[11px] text-zinc-400 dark:text-zinc-500">
          De-risk the business and increase knowledge velocity toward PMF
        </p>
      </div>
      {!isGuest && (
        <button
