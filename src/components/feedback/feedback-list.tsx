'use client';

import { useState, useEffect, useRef } from 'react';
import { Search } from 'lucide-react';
import { trpc } from '@/lib/trpc';
import { useFeedbackStore } from '@/lib/stores/feedback-store';
import { FeedbackStatusBadge } from './feedback-status-badge';
import { FeedbackTypeBadge } from './feedback-type-badge';
import { Avatar } from '@/components/shared/avatar';
import {
  BRAND_AMBER,
  FEEDBACK_TYPE_LABELS,
  FEEDBACK_STATUS_LABELS,
  PRIORITY_LABELS,
} from '@/lib/constants';
import type { FeedbackStatus, FeedbackType, Priority } from '@/generated/prisma/client';

interface FeedbackListProps {
  workspaceId: string;
  members: Array<{ id: string; name: string; avatarUrl?: string | null; avatarColor?: string }>;
}

const STATUS_TABS = [
  { value: null, label: 'All' },
  { value: 'OPEN', label: 'Open' },
  { value: 'IN_PROGRESS', label: 'In Progress' },
  { value: 'RESOLVED', label: 'Resolved' },
  { value: 'CLOSED', label: 'Closed' },
] as const;

export function FeedbackList({ workspaceId, members }: FeedbackListProps) {
  const {
    selectedFeedbackId,
    setSelectedFeedbackId,
    statusFilter,
    setStatusFilter,
    typeFilter,
    setTypeFilter,
    priorityFilter,
    setPriorityFilter,
    assigneeFilter,
    setAssigneeFilter,
  } = useFeedbackStore();

  const [searchInput, setSearchInput] = useState('');
  const [debouncedSearch, setDebouncedSearch] = useState('');
  const searchTimer = useRef<ReturnType<typeof setTimeout>>();

  useEffect(() => {
    if (searchTimer.current) clearTimeout(searchTimer.current);
    searchTimer.current = setTimeout(() => {
      setDebouncedSearch(searchInput);
    }, 300);
    return () => {
      if (searchTimer.current) clearTimeout(searchTimer.current);
    };
  }, [searchInput]);

  const { data: stats } = trpc.feedback.getStats.useQuery(
    { workspaceId },
    { enabled: !!workspaceId }
  );

  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    trpc.feedback.list.useInfiniteQuery(
      {
        workspaceId,
        status: (statusFilter || undefined) as FeedbackStatus | undefined,
        type: (typeFilter || undefined) as FeedbackType | undefined,
        priority: (priorityFilter || undefined) as Priority | undefined,
        assigneeId: assigneeFilter ?? undefined,
        search: debouncedSearch || undefined,
        limit: 30,
      },
      {
        enabled: !!workspaceId,
        getNextPageParam: (lastPage) => lastPage.nextCursor ?? undefined,
      }
    );

  const feedbackItems = data?.pages.flatMap((p) => p.items) ?? [];
  const hasActiveFilters = statusFilter !== null || typeFilter !== null || priorityFilter !== null || assigneeFilter !== null || debouncedSearch !== '';

  return (
    <div className="flex h-full flex-col">
      {/* Stats summary */}
      {stats && stats.totalCount > 0 && (
        <div className="flex items-center gap-3 border-b border-zinc-200/60 dark:border-zinc-800 px-5 py-2 text-[11px] text-zinc-500 dark:text-zinc-400">
          <span>{stats.totalCount} item{stats.totalCount !== 1 ? 's' : ''}</span>
          {Object.entries(stats.byStatus).map(([status, count]) => (
            <span key={status} className="flex items-center gap-1">
              <span className="font-medium">{count as number}</span> {FEEDBACK_STATUS_LABELS[status] ?? status}
            </span>
          ))}
        </div>
      )}

      {/* Filters */}
      <div className="flex flex-wrap items-center gap-2 border-b border-zinc-200/60 dark:border-zinc-800 px-5 py-2.5">
        {/* Status tabs */}
        <div className="flex items-center gap-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800/60 p-0.5" role="tablist" aria-label="Filter by status">
          {STATUS_TABS.map((tab) => (
            <button
              key={tab.label}
              onClick={() => setStatusFilter(tab.value)}
              role="tab"
              aria-selected={statusFilter === tab.value}
              className={`rounded px-2.5 py-1 text-[11px] font-medium transition-colors ${
                statusFilter === tab.value
                  ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 shadow-sm'
                  : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300'
              }`}
            >
              {tab.label}
            </button>
          ))}
        </div>

        {/* Type filter */}
        <select
          value={typeFilter ?? ''}
          onChange={(e) => setTypeFilter(e.target.value || null)}
          className="rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-2 py-1 text-[11px] text-zinc-600 dark:text-zinc-300 outline-none"
          aria-label="Filter by type"
        >
          <option value="">All types</option>
          {Object.entries(FEEDBACK_TYPE_LABELS).map(([key, label]) => (
            <option key={key} value={key}>{label}</option>
          ))}
        </select>

        {/* Priority filter */}
        <select
          value={priorityFilter ?? ''}
          onChange={(e) => setPriorityFilter(e.target.value || null)}
          className="rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-2 py-1 text-[11px] text-zinc-600 dark:text-zinc-300 outline-none"
          aria-label="Filter by priority"
        >
          <option value="">All priorities</option>
          {Object.entries(PRIORITY_LABELS).map(([key, label]) => (
            <option key={key} value={key}>{label}</option>
          ))}
        </select>

        {/* Assignee filter */}
        <select
          value={assigneeFilter ?? ''}
          onChange={(e) => setAssigneeFilter(e.target.value || null)}
          className="rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-2 py-1 text-[11px] text-zinc-600 dark:text-zinc-300 outline-none hidden md:block"
          aria-label="Filter by assignee"
        >
          <option value="">All assignees</option>
          {members.map((m) => (
            <option key={m.id} value={m.id}>{m.name}</option>
          ))}
        </select>

        {/* Search */}
        <div className="relative ml-auto">
          <Search className="absolute left-2 top-1/2 h-3 w-3 -translate-y-1/2 text-zinc-400" />
          <input
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            placeholder="Search feedback..."
            className="rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 pl-7 pr-2 py-1 text-[11px] text-zinc-700 dark:text-zinc-300 outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600 w-48"
            aria-label="Search feedback"
          />
        </div>
      </div>

      {/* Table header */}
      <div
        className="grid grid-cols-[60px_1fr_90px_90px] md:grid-cols-[60px_90px_1fr_90px_80px_80px_90px] gap-2 border-b border-zinc-200/60 dark:border-zinc-800 px-5 py-2 text-[10px] uppercase tracking-wider text-zinc-400 dark:text-zinc-500"
        role="row"
      >
        <span role="columnheader">FB #</span>
        <span role="columnheader" className="hidden md:block">Type</span>
        <span role="columnheader">Title</span>
        <span role="columnheader">Status</span>
        <span role="columnheader" className="hidden md:block">Priority</span>
        <span role="columnheader" className="hidden md:block">Assignee</span>
        <span role="columnheader" className="hidden md:block">Created</span>
      </div>

      {/* Rows */}
      <div className="flex-1 overflow-y-auto" role="table" aria-label="Feedback list">
        {isLoading ? (
          <div className="flex items-center justify-center py-12 text-xs text-zinc-400">
            Loading feedback...
          </div>
        ) : feedbackItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-zinc-400 dark:text-zinc-500">
            {hasActiveFilters ? (
              <>
                <p className="text-sm font-medium">No matching feedback</p>
                <p className="mt-1 text-xs">Try adjusting your filters or search query</p>
              </>
            ) : (
              <>
                <p className="text-sm font-medium">No feedback yet</p>
                <p className="mt-1 text-xs">Submit your first bug report, feature request, or question</p>
              </>
            )}
          </div>
        ) : (
          <>
            {feedbackItems.map((item) => {
              const isClosed = item.status === 'CLOSED' || item.status === 'RESOLVED';
              const isSelected = selectedFeedbackId === item.id;

              return (
                <button
                  key={item.id}
                  onClick={() => setSelectedFeedbackId(isSelected ? null : item.id)}
                  role="row"
                  aria-selected={isSelected}
                  className={`grid w-full grid-cols-[60px_1fr_90px_90px] md:grid-cols-[60px_90px_1fr_90px_80px_80px_90px] gap-2 border-b border-zinc-100 dark:border-zinc-800/60 px-5 py-2.5 text-left text-xs transition-colors ${
                    isSelected
                      ? 'bg-amber-50/60 dark:bg-amber-900/10'
                      : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/40'
                  } ${isClosed ? 'opacity-50' : ''}`}
                >
                  {/* FB # */}
                  <div className="flex items-center">
                    <span className="font-mono text-[11px] text-zinc-500 dark:text-zinc-400">
                      {item.identifier}
                    </span>
                  </div>

                  {/* Type (hidden on mobile) */}
                  <div className="hidden md:flex items-center">
                    <FeedbackTypeBadge type={item.type} />
                  </div>

                  {/* Title */}
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className="truncate font-medium text-zinc-900 dark:text-zinc-100">
                      {item.title}
                    </span>
                    {item.description && (
                      <span className="truncate text-[11px] text-zinc-400 dark:text-zinc-500">
                        {item.description.slice(0, 120)}
                      </span>
                    )}
                  </div>

                  {/* Status */}
                  <div className="flex items-center">
                    <FeedbackStatusBadge status={item.status} />
                  </div>

                  {/* Priority (hidden on mobile) */}
                  <div className="hidden md:flex items-center text-zinc-500 dark:text-zinc-400">
                    {item.priority !== 'NONE' ? (
                      <span className="truncate text-[11px]">{PRIORITY_LABELS[item.priority] ?? item.priority}</span>
                    ) : (
                      <span className="text-zinc-300 dark:text-zinc-600">—</span>
                    )}
                  </div>

                  {/* Assignee (hidden on mobile) */}
                  <div className="hidden md:flex items-center gap-1.5">
                    {item.assignee ? (
                      <>
                        <Avatar
                          name={item.assignee.name}
                          avatarUrl={item.assignee.avatarUrl}
                          avatarColor={item.assignee.avatarColor ?? undefined}
                          size="xs"
                        />
                        <span className="truncate text-zinc-500 dark:text-zinc-400">
                          {item.assignee.name.split(' ')[0]}
                        </span>
                      </>
                    ) : (
                      <span className="text-zinc-300 dark:text-zinc-600">—</span>
                    )}
                  </div>

                  {/* Created (hidden on mobile) */}
                  <div className="hidden md:flex items-center text-[11px] text-zinc-500 dark:text-zinc-400">
                    {new Date(item.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </div>
                </button>
              );
            })}

            {/* Load More */}
            {hasNextPage && (
              <div className="flex justify-center py-3">
                <button
                  onClick={() => fetchNextPage()}
                  disabled={isFetchingNextPage}
                  className="rounded-md px-4 py-1.5 text-xs font-medium transition-colors hover:opacity-90 disabled:opacity-50"
                  style={{ backgroundColor: BRAND_AMBER, color: '#fff' }}
                >
                  {isFetchingNextPage ? 'Loading...' : 'Load more'}
                </button>
              </div>
            )}
          </>
        )}
      </div>
    </div>
  );
}
